...
- **Risks** — things that go wrong silently:
  ...
  - **`@mui/x-date-pickers` v8+ theme isolation**: `@mui/x-date-pickers` v8 introduced separate `MuiPickersTextField`/`MuiPickersInput` components that are **independent** from `MuiTextField`/`MuiInput`. Any theme overrides on `MuiTextField` (including `defaultProps.variant`, font styles) and `MuiInput` will no longer apply to date pickers. The pickers default to `variant: 'outlined'` regardless of the theme's `MuiTextField.defaultProps`. Build and tests still pass — it is a visual-only regression that only Storybook or a browser will catch.
...
5. **Apply manual fixes** for everything codemods can't handle. Be especially careful with:
   ...
   - **Backwards-compatible props forwarded to slot props**: when a legacy prop (e.g., `InputLabelProps`, `InputProps`) is kept for backwards compatibility and internally spread into `slotProps.*`, the legacy prop type carries a `ref` typed for a DOM element (e.g., `Ref<HTMLLabelElement>`) while the slot expects a component-instance ref (e.g., `Ref<Component<InputLabelProps>>`). These are incompatible and will fail `build-test` even though the runtime behaviour is correct. Fix: add `Omit<..., 'ref'>` to the legacy prop type in `types.ts` (e.g., `Omit<Partial<MuiInputLabelProps>, 'ref'>`). After any migration that introduces slot props while preserving old-style props, run `yarn build-test` (or `tsc --noEmit`) to surface these mismatches early.
   ...
   - **`@mui/x-date-pickers` theme overrides** (applies when upgrading from v7 to v8+): check every theme file for `MuiTextField.defaultProps` (especially `variant`), `MuiTextField.styleOverrides`, and `MuiInput.styleOverrides`. Each must be mirrored to its picker-specific equivalent: `MuiPickersTextField` and `MuiPickersInput`. Without this, date pickers silently revert to `variant: 'outlined'` and lose any custom font/weight styling. Also add `import type {} from '@mui/x-date-pickers/themeAugmentation'` to the theme override file — without this side-effect import, TypeScript will reject the new keys with "does not exist in type". After adding the overrides, confirm visually via Storybook MCP that the date picker stories render identically to before the upgrade.
